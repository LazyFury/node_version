...
const binp = path.join(cargoDir, "bin", "node_version");

if (fs.existsSync(binp)) {
  console.log("Uninstalling node_version...");
  exec(`cargo uninstall node_version`, (error, stdout, stderr) => {
    console.log(stdout);
    if (error || stderr) {
  ...
  console.log("node_version not found skipping!");
}
